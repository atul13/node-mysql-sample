...
class Otp{
	...
	async updateNewOtp(userId, code, channel){

		let table = this.table;

		return await new Promise((resolve, reject) => {
			db.query('UPDATE ?? SET code = ? , channel = ?, status = 0  WHERE  userId = ? ', [table, code, channel, userId], function (error, results, fields) {
			  if (error) reject(error);
	...
	async updaterevOtp(code, userId){

		let table = this.table;

		return await new Promise((resolve, reject) => {
			db.query('UPDATE ?? SET status = 0 WHERE code = ? AND userId = ? ', [table, code, userId], function (error, results, fields) {
			  if (error) reject(error);
			  return resolve(results);
			});
		});

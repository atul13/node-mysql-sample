...
class ScriptAttributes{
	...
	async getAttributesByIds(sessionScriptIds) {
        ...
        return await new Promise((resolve, reject) => {
        	...
        	db.query('SELECT * FROM ?? WHERE sessionScriptId IN (?) AND status = 1  ORDER BY id ASC', [table, sessionScriptIds], function (error, results, fields) {
	...
	async updateScriptAttribute(data) {
		let table = this.table;
		//console.log('---------lalitdata-------------',data)
        return await new Promise((resolve, reject) => {
        	
        	db.query('UPDATE ?? SET status = 0 WHERE sessionScriptId = ? AND status = 1', [table, data], function (error, results, fields) {
			  if (error) reject(error);
			 // console.log('---------lalitattribute-------------',results)
